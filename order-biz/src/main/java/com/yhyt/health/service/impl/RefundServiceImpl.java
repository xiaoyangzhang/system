...
//		order.setState((byte) 2);
		order.setState((byte) 6);
		order.setUpdateTime(new Date());
		int i2 = this.orderMapper.updateAndCheck(order, params);
		params.clear();
		params.put("type", 4);
		params.put("orderId", orderList.get(0).getId());
		OrderDetail orderDetail = new OrderDetail();
		orderDetail.setType((byte) 1);
		int count2 = this.orderDetailMapper.updateAndCheck(orderDetail, params);
...
//		payDetail.setSend("");
	...
	@Async
	public AppResult weixinRefund(PayDetail payDetail) {
...
//					params1.put("orderNo",payDetail.getOrderid());
					...
					try {
						...
						OrderVo order = orderMapper.selectByOrderNo(payDetail.getOrderid());
						messageService.sendMessage(order.getPatientId(),"1","新健康","您有"+order.getHospital()+order.getDepartment()+order.getItemName()+"服务退款已成功，请查看。");
		...
		OrderVo order = orderMapper.selectByOrderNo(payDetail.getOrderid());
		messageService.sendMessage(order.getPatientId(),"1","新健康","您有"+order.getHospital()+order.getDepartment()+order.getItemName()+"服务退款失败，请联系客服电话：400-819-0503。");
		return result;
	}
